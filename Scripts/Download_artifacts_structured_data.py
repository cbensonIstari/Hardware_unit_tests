    ...
    for index, row in metadata_df.iterrows():
        ...
        if file_extension in viewable_extensions:
            if download_url and 'null' not in download_url:  # Ensure the download URL exists
                try:
                    download_artifact(artifact_name, download_url, file_extension)
                except Exception as e:
                    print(f"Error downloading {artifact_name}: {e}")
            else:
                # If download link is expired, re-fetch the artifact metadata
                print(f"Download link for {artifact_name} is expired or missing. Fetching metadata again...")
